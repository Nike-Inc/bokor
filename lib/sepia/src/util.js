...
function constructAndCreateFixtureFolder(reqUrl, reqHeaders) {
  ...
  if (!usesGlobalFixtures(reqUrl)){
    if (reqHeaders['x-bokor-test-name']) {
      testFolder = reqHeaders['x-bokor-test-name'];
  ...
  var subFolder = url.parse(reqUrl);
  var x = subFolder.pathname.split('/');
  subFolder = subFolder.hostname + '/' + x[1];
  var folder = path.resolve(globalOptions.filenamePrefix, testFolder, subFolder);
  mkdirpSync(folder);
